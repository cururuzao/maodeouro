...
const TemplatesPage = () => {
  ...
  const [templates, setTemplates] = useState<Template[]>(() => {
    try { return JSON.parse(localStorage.getItem("templates") || "[]"); } catch { return []; }
  });

  const [folders] = useState([{ name: "Todos", count: templates.length }]);

  const handleCreate = () => {
    if (!name.trim() || !content.trim()) return;
    const newT: Template = { name, type, content, instances: 0 };
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div>
            ...
            <div className="space-y-1">
              {folders.map((f) => (
                <button key={f.name} className="w-full flex items-center justify-between px-3 py-2 rounded-lg text-sm bg-primary/10 text-primary border-l-2 border-primary">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-border text-muted-foreground text-xs">
                  ...
                  <th className="text-left py-3 px-4 font-medium">Instâncias</th>
                </tr>
              </thead>
              <tbody>
                {templates.length === 0 ? (
                  ...
                  templates.map((t, i) => (
                    <tr key={i} className="border-b border-border hover:bg-secondary/50 cursor-pointer" onClick={() => setSelectedTemplate(t)}>
                      ...
                      <td className="py-3 px-4 text-muted-foreground">{t.instances} instâncias</td>
                    </tr>
                  ))
                )}
              </tbody>
